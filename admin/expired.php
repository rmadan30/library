        ...
        <div class="container">
            ...
            <?php
                if(isset($_POST['submit'])){
                    ...
                    //query to select data from issue book matching the username  
                    $result=mysqli_query($db,"SELECT * FROM `issue_book` where username='$_POST[username]' and bid='$_POST[bid]';");
                    ...
                    //get todays date
                    $x= date("Y-m-d");
                    //day on which they have to return
                    //query to insert value in our fine table
                    mysqli_query($db,"INSERT INTO `fine` VALUES ('$_POST[username]', '$_POST[bid]', '$x', '$day', '$fine', 'not paid');");
                    
                    $var1='<p style="color: yellow; background-color: green;"> RETURNED </p>';
                    mysqli_query($db, "UPDATE `issue_book` SET `status`='$var1' where username='$_POST[username]' and bid='$_POST[bid]'");
            ...
            <?php
                ...
                if(isset($_SESSION['login_user'])){
                    ...
                    if(isset($_POST['submit2'])){
                        $sql="SELECT student.username,rollnumber, books.bid,name,authors,edition,issue_book.status,issue,issue_book.return FROM student INNER JOIN issue_book on student.username= issue_book.username INNER JOIN books on issue_book.bid=books.bid where issue_book.status ='$ret' order by `issue_book`.`return` DESC;";
                    ...
                    // condition changed checked if the exp variable is used
                        ...
                        $sql="SELECT student.username,rollnumber, books.bid,name,authors,edition,issue_book.status,issue,issue_book.return FROM student INNER JOIN issue_book on student.username= issue_book.username INNER JOIN books on issue_book.bid=books.bid where issue_book.status ='$exp' order by `issue_book`.`return` DESC;";
                        $result=mysqli_query($db,$sql);
                    }
